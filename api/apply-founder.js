...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (result && result.dryRun) {
      console.warn('[apply-founder] DRY RUN — RESEND_API_KEY is not set in environment');
      return res.status(503).json({
        error: 'Email service not configured. Please set RESEND_API_KEY in Vercel environment variables and redeploy.',
      });
    }
    console.log('[apply-founder] sent successfully', { project: data.projectName, email: data.email });
    return res.status(200).json({ ok: true });
  } catch (e) {
    console.error('[apply-founder] sendEmail failed:', e.message);
    // Surface a more helpful hint when the typical Resend errors hit
    const msg = e.message || '';
